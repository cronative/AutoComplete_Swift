...
//    })
    ...
    let theRequest = URLRequest(url: URL(string: "\(url)?\(query(params as [String : AnyObject]))")!)
    let task = session.dataTask(with: theRequest, completionHandler: { (data:Data?, response:URLResponse?, error:Error?) in
